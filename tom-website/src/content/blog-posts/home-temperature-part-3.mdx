...
This function constructs a SQL query to select readings from the specified table between the given start and end dates. It scans the results into a slice of `Reading` structs, then converts them into `APIReading` structs for easier consumption by the API. I should abstract that conversion out somewhere, but for now, this will do.
