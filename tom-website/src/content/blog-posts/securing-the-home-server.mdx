...
The above configuration defines a Podman container for the Minecraft server. The container is based on the `itzg/minecraft-server` image, which is (at the time of writing this) a very popular image for running Minecraft servers in containers. A port config passes the host communication for Minecraft through to the podman container. The container is also configured to run as a non-root user, which further limits the potential damage that can be done if the container is compromised.

To keep the files for the worlds between running the container, a volume is mapped from the host path `/var/lib/minecraft-worlds/world-files` to the container directory `/data`. This allows the Minecraft server to read and write world files to the host filesystem, while limiting its access to other parts of the host filesystem. This is the big benefit to the containerisation approach. If there happened to be another Log4j-like vulnerability in Java, and Minecraft was vulnerable, the attacker would only be able to access the files in `/var/lib/minecraft-worlds/world-files`, and nothing else on the host system.
